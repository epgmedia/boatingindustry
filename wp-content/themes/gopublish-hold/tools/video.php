...
<?php global $post; $feature_photo = get_post_meta($post->ID, feature_photo, true); if (has_post_thumbnail()) 
	...
	else if ($feature_photo) { ?><a href="<?php the_permalink(); ?>"><img src="<?php echo $feature_photo; ?>" class="catboxthumb" style="float:<?php echo $thumbplacement; ?>;margin-bottom:10px;margin-<?php echo $thumbplacement; ?>:0px;width:90px" /></a><?php } 
	else 
...
$attachment = wp_get_attachment_url($attachment_id); if ($attachment) { ?><a href="<?php the_permalink(); ?>"><img src="<?php echo $attachment; ?>" class="catboxthumb" style="float:<?php echo $thumbplacement; ?>;margin-bottom:10px;margin-<?php echo $thumbplacement; ?>:0px;width:90px" /></a><?php } }
...
<div <?php if ($customcolors==on) { ?>style="background-color:<?php echo $instance['header-color']; ?> !important;"<?php } ?>class="widgetfooter<?php if ($instance['widget-style']=="Style 3") { ?>3<?php } else { ?>1<?php } ?>"></div></div>
	...
	function form($instance) { 
		...
		<?php if ($sidebarname == "Home Narrow Column") { $videowidth=get_option('home_narrow_column'); } ?>
		<?php if ($sidebarname == "Home Wide Column") { $videowidth=get_option('home_wide_column'); } ?>
		<?php if ($sidebarname == "Home Full Width Column") { $videowidth=get_option('home_full_width_column'); } ?>
