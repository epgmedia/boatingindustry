...
<?php if ( have_posts() ) : while ( have_posts() ) : the_post(); ?>
	<?php $teamschedulecheck = get_post_meta( $post->ID, opponent, true );
	$staffposition           = get_post_meta( $post->ID, name, true ); ?>
<?php endwhile;
else: ?>
...
<?php if ( $staffposition ) {
	include( TEMPLATEPATH . "/staff.php" );
} else { ?>
	<?php if ( $teamschedulecheck ) {
		include( TEMPLATEPATH . "/teamschedule.php" );
	} else { ?>
			...
			<div id="contentleft">
				...
				<div class="postarea">
					...
					<?php if ( have_posts() ) : while ( have_posts() ) : the_post(); ?>
						...
						<?php $slideshowwidth = get_theme_mod( 'mm-width' );
						$slideshowheight      = get_theme_mod( 'mm-height' );
						$sswidth              = $slideshowwidth;
						$ssheight             = $slideshowheight;
						...
						<?php $mmcheck = get_option( 'msno' );
						if ( ( $mmcheck == "msno402841m" ) && ( get_theme_mod( 'mm-display' ) == "Above Story" ) ) { ?>
							<?php if ( $video ) { ?>
								...
								<div style="margin-bottom:15px">
									<?php echo $video1; ?>
									<?php if ( $videographer ) { ?>
										<p class="photocredit" style="padding-bottom:0px">Video Credit: <?php echo $videographer; ?></p><?php } ?>
								</div>
							<?php } ?>
						<?php } ?>
						...
						<div id="permalinkphotobox">
							...
							<?php if ( $teasertitle ) { ?>
								...
								<?php if ( $photographer ) { ?>
									<p class="photocredit">Photo Credit: <?php echo $photographer; ?></p><?php } ?>
								<?php if ( $caption ) { ?>
									<p class="photocaption"><?php echo $caption; ?></p><?php } else { ?>
									<p style="font-size:1px">.</p><?php } ?>
							...
							<?php } else { ?>
								...
								<?php if ( ( $mmcheck == "msno402841m" ) && ( get_theme_mod( 'mm-display' ) == "Overlay" ) ) { ?>
									...
									<?php if ( $video ) { ?>
										...
										<div class="videoplay2">
											<a href="#TB_inline?inlineId=media<?php echo $mediacount; ?>&height=<?php echo $ssheight; ?>&width=<?php echo $sswidth; ?>" title="<?php the_title(); ?>" class="thickbox" style="font-size:22px;line-height:30px;">Watch Video</a>
										</div>
										...
										<div id="media<?php echo $mediacount; ?>" style="display: none;">
											<?php if ( $video ) {
												...
												$pattern = "/width=\"[0-9]*\"/";
												$video   = preg_replace( $pattern, "width=' " . $videowidth . " '", $video );
												echo $video;
											} ?>
											<?php $videographer = get_post_meta( $post->ID, videographer, true );
											if ( $videographer ) { ?>
												<div class="mmdivider">
												<p class="photographer">Video Credit: <?php echo $videographer; ?></p>
												</div><?php } ?>
										</div>
									<?php } ?>
									...
									<?php if ( $slideshow ) {
										...
										<div class="videoplay2">
											<a href="#TB_inline?inlineId=media<?php echo $mediacount; ?>&height=<?php echo $ssheight; ?>&width=<?php echo $sswidth; ?>" title="<?php the_title(); ?>" class="thickbox" style="font-size:22px;line-height:30px;">Watch Slideshow</a>
										</div>
										<div style="clear:both"></div>
										<div id="media<?php echo $mediacount; ?>" style="display: none;">
											<?php if ( $slideshow ) {
											...
											if ( $slideshowcredit ) { ?>
												<div class="mmdivider">
												<p class="photographer">Slideshow Credit: <?php echo $slideshowcredit; ?></p>
												</div><?php } ?>
										</div>
									<?php } ?>
									...
									<?php if ( $soundslides ) { ?>
										...
										<div id="media<?php echo $mediacount; ?>" style="display: none;">
											...
											if ( $soundslidescredit ) { ?>
												<div class="mmdivider">
												<p class="photographer">Slideshow Credit: <?php echo $soundslidescredit; ?></p>
												</div><?php } ?>
										</div>
									<?php } ?>
								...
								<?php if ( $gallery ) {
									...
									echo do_shortcode( $showalbum ); ?><?php } ?>

								<?php if ( $slideshowcredit ) { ?>
									<p class="photocredit">Credit Credit: <?php echo $slideshowcredit; ?></p><?php } ?>
							...
							/* COMMENTED OUT RELATED CONTENT WITHIN STORY TEXT
							$related = get_post_meta( $post->ID, related, true );
							if ( $related != "No" ) { ?>
						...
						<p>
							<?php snowriter(); ?>
							<?php the_time( 'F j, Y' ); ?>  <?php edit_post_link( '(Edit)', '', '' ); ?>
	...
	<?php }
} ?><!--end of else from sports schedule check and staff check at top-->
