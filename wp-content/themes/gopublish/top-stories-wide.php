...
<div id="slideshow">
		...
		<div class="cycle">
				...
				<?php $pattern = "/height=\"[0-9]*\"/"; $video = preg_replace($pattern, "height='300'", $video); $pattern = "/width=\"[0-9]*\"/"; $video = preg_replace($pattern, "width='938'", $video); ?><div style="float:left;width:938px;"><?php echo $video; ?></div>

			<?php } else { ?>

			<?php global $post; $feature_photo = get_post_meta($post->ID, feature_photo, true); if (has_post_thumbnail()) 
			{ the_post_thumbnail( 'topstories', array('class' => 'widesliderimage'));  } 
	else if ($feature_photo) { ?><a href="<?php if (($customlink) && (get_theme_mod('top-stories-links')=="Yes")) { echo $customlink; } else { the_permalink(); } ?>"><img src="<?php echo $feature_photo; ?>" class="widesliderimage" style="width:938px;" /></a><?php } 
	else 
...
$attachment = wp_get_attachment_url($attachment_id); if ($attachment) { ?><a href="<?php if (($customlink) && (get_theme_mod('top-stories-links')=="Yes")) { echo $customlink; } else { the_permalink(); } ?>"><img src="<?php echo $attachment; ?>" class="widesliderimage" style="width:938px;" /></a><?php } } ?>
