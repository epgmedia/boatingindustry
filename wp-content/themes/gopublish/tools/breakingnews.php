...
<?php $count = 1; ?>
<?php $recent = new WP_Query("cat=" . get_theme_mod('breakingnews-cat') . "&showposts=" . get_theme_mod('breakingnews-scrollbox')); while($recent->have_posts()) : $recent->the_post();?><?php $headline = get_post_meta($post->ID, breakingnewsheadline, true); $teaser = get_post_meta($post->ID, breakingnewsteaser, true); $breakingnewslink = get_post_meta($post->ID, breakingnewslink, true); ?>
