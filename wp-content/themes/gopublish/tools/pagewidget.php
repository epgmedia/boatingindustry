...
class sno_page extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		if ( $instance['page'] == "" ) {
			...
			$categoryname = get_the_title( $instance['page'] );
			$categoryslug = get_permalink( $instance['page'] );
			...
			while ( $the_query->have_posts() ) : $the_query->the_post();
				...
				if ( ( $photodisplaywidth == get_option( 'home_right_column' ) ) && ( $instance['sidebarname'] == "Home Sidebar" ) ) {
					$marginkey = 3;
				}
				if ( ( $photodisplaywidth == get_option( 'home_narrow_column' ) ) && ( $instance['sidebarname'] == "Home Bottom Narrow" ) ) {
					$marginkey = 3;
				}
				if ( ( $photodisplaywidth == get_option( 'non_home_right_column' ) ) && ( $instance['sidebarname'] == "Non-Home Sidebar" ) ) {
					$marginkey = 3;
				}
				if ( ( $photodisplaywidth == get_option( 'non_home_right_column' ) ) && ( $instance['sidebarname'] == "Sports Center Sidebar" ) ) {
					$marginkey = 3;
				}
				if ( ( $photodisplaywidth == get_option( 'home_left_column' ) ) && ( $instance['sidebarname'] == "Home Bottom Left" ) ) {
					$marginkey = 3;
				}
				if ( ( $photodisplaywidth == get_option( 'home_center_column' ) ) && ( $instance['sidebarname'] == "Home Bottom Right" ) ) {
					$marginkey = 3;
				}
				if ( ( $photodisplaywidth == get_option( 'home_wide_column' ) ) && ( $instance['sidebarname'] == "Home Bottom Wide" ) ) {
					$marginkey = 3;
				}
				...
				<?php if ( ( $instance['category-photo-size'] == "Large" ) && ( has_post_thumbnail() ) ) {
					...
					href="<?php the_permalink(); ?>"><img src="<?php echo $catimage[0]; ?>" style="width:100%"
														  class="catboxphoto" alt="<?php the_title(); ?>" /></a><?php
				...
				<span
					style="font-size:<?php echo $headlinesize; ?>px; line-height:<?php echo $headlineheight; ?>px;"><?php $teaser = $instance['category-teaser'];
					if ( $teaser ) {
						the_content_limit( $teaser, "<span class='readmore'>Read More</span>" );
					} ?></span>
			...
			if ( $instance['view-all'] == 'on' ) {
				...
				<a href="<?php echo cat_id_to_slug( $instance['category'] ); ?>"><p class="viewall">View Page <span
	...
	function form( $instance ) {
		...
		if ( $sidebarname == "Home Sidebar" ) {
			$photowidth = get_option( 'home_right_column' );
		}
		if ( ( $sidebarname == "Non-Home Sidebar" ) || ( $sidebarname == "Sports Center Sidebar" ) ) {
			$photowidth = get_option( 'non_home_right_column' );
		}
		if ( $sidebarname == "Home Bottom Narrow" ) {
			$photowidth = get_option( 'home_narrow_column' );
		}
		if ( $sidebarname == "Home Bottom Wide" ) {
			$photowidth = get_option( 'home_wide_column' );
		}
		if ( $sidebarname == "Home Main Column" ) {
			$photowidth = get_option( 'home_right_column' );
		}
		if ( $sidebarname == "Home Bottom Left" ) {
			$photowidth = get_option( 'home_left_column' );
		}
		if ( $sidebarname == "Home Bottom Right" ) {
			$photowidth = get_option( 'home_center_column' );
		}
		if ( $photowidth ) {
		...
		<div style="float:left;width:230px;margin-right:20px;border-right:1px solid #aaaaaa;padding-right:10px;">
			...
			<p>
				...
				<select id="<?php echo $this->get_field_id( 'headline-size' ); ?>"
					...
					<option value="12" <?php if ( '12' == $instance['headline-size'] ) {
						echo 'selected="selected"';
					} ?>>12
					</option>
			...
			<input type="hidden" id="<?php echo $this->get_field_id( 'photowidth' ); ?>"
				   name="<?php echo $this->get_field_name( 'photowidth' ); ?>"
				   value="<?php echo $instance['photowidth']; ?>" />
		...
		<div style="float:left;width:210px">
			...
			<p>
				<input class="colorwellpage<?php echo $number; ?>"
			...
			<script>
				...
				jQuery(document).ready(function () {
					...
					jQuery(".colorwellpage<?php echo $number; ?>").click(function () {
						jQuery('#snocolorpickerpage<?php echo $number; ?>').slideDown()
					});
				});
			</script>
