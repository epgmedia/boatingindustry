	...
	function widget($args, $instance) {
		...
		$categoryslug = cat_id_to_slug($instance['category']); $categoryname = cat_id_to_name($instance['category']);$customcolors=$instance['custom-colors'];
            ...
            if ($count <= $instance['number']) {              
				...
				if (($instance['category-photo-size'] == "Large") && ($instance['sidebarname'] == "Home Wide Column")) { $photodisplaywidth = $instance['photowidth']; $thumbnailsize="home400"; }
				else if (($instance['category-photo-size'] == "Large") && ($instance['sidebarname'] == "Home Full Width Column")) { $photodisplaywidth = $instance['photowidth']; $thumbnailsize="permalink"; }
				...
				else if (($instance['category-photo-size'] == "Large") && ($instance['sidebarname'] == "Home Narrow Column")) { $photodisplaywidth = $instance['photowidth']; $thumbnailsize="home160"; }
				...
				if (($instance['category-photo-size'] == "Small") && ($instance['sidebarname'] == "Home Wide Column")) { $photodisplaywidth = get_option('home_narrow_column'); $thumbnailsize="home160"; } 
				else if (($instance['category-photo-size'] == "Small") && ($instance['sidebarname'] == "Home Full Width Column")) { $photodisplaywidth = get_option('home_narrow_column'); $thumbnailsize="home160"; } 
				else if (($instance['category-photo-size'] == "Small") && ($instance['sidebarname'] == "Home Narrow Column")) { $photodisplaywidth = get_option('home_narrow_column'); $thumbnailsize="home160"; } 
				else if ($instance['category-photo-size'] == "Small") { $photodisplaywidth = 120; $thumbnailsize="home120"; } 
				
				if (($instance['category-photo-size'] == "Medium") && ($instance['sidebarname'] == "Home Wide Column")) { $photodisplaywidth = 200; $thumbnailsize="archive"; } 
				else if (($instance['category-photo-size'] == "Medium") && ($instance['sidebarname'] == "Home Full Width Column")) { $photodisplaywidth = 200; $thumbnailsize="archive"; } 
				...
				if (($photodisplaywidth==get_option('home_narrow_column')) && ($instance['sidebarname']=="Home Narrow Column")) { $marginkey = 3; }
				...
				if (($photodisplaywidth==get_option('home_wide_column')) && ($instance['sidebarname']=="Home Wide Column")) { $marginkey = 3; }

				$photocheck = 0; if (has_post_thumbnail()) $photocheck = 5; $feature_photo = get_post_meta($post->ID, feature_photo, true); if ($feature_photo) $photocheck = 5; global $wpdb; $attachment_id = $wpdb->get_var("SELECT ID FROM $wpdb->posts WHERE post_parent = '$post->ID' AND post_status = 'inherit' AND post_type='attachment' ORDER BY post_date DESC LIMIT 1"); $attachment = wp_get_attachment_url($attachment_id); if ($attachment) $photocheck = 5;
...
<?php if (($instance['category-photo-size'] == "Large") && ($attachment)) { ?><a href="<?php the_permalink(); ?>"><img src="<?php echo bloginfo('template_url'); ?>/timthumb.php?src=<?php echo $attachment; ?>&w=400&q=100" style="width:100%" class="catboxphoto" alt="<?php the_title(); ?>" /></a><?php }

	else if (has_post_thumbnail()) 
			{ the_post_thumbnail( $thumbnailsize, array('class' => 'catboxphoto'));} 
	else if ($feature_photo) { ?><a href="<?php the_permalink(); ?>"><img src="<?php echo $feature_photo; ?>" class="catboxphoto" style="width:<?php echo $photodisplaywidth; ?>px" /></a><?php } 
	else 
			{ if ($attachment) { ?><a href="<?php the_permalink(); ?>"><img src="<?php echo $attachment; ?>" class="catboxphoto" style="width:<?php echo $photodisplaywidth; ?>px" /></a><?php } } ?>
						
					
						<?php if ($instance['show-caption']==on) {
            ...
            <?php $writer = get_post_meta($post->ID, writer, true); if ($writer) { ?><p class="writer"><?php snowriter(); ?></p> <?php } 
...
<?php	}
	...
	function form($instance) { 
		...
		if ($sidebarname == "Home Narrow Column") { $photowidth=get_option('home_narrow_column'); }
		if ($sidebarname == "Home Wide Column") { $photowidth=get_option('home_wide_column'); }
		if ($sidebarname == "Home Full Width Column") { $photowidth=get_option('home_right_column'); }
