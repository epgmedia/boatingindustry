...
class sno_sports_scrollbox extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		$side_bar_thumbnail = array(
			'Non-Home Sidebar'   => 'permalink',
			'Home Main Column'   => 'permalink',
			'Home Bottom Left'   => 'home280',
			'Home Bottom Narrow' => 'home160',
			'Home Bottom Right'  => 'home280',
			'Home Bottom Wide'   => 'home400',
			'Home Sidebar'       => 'permalink',
		);
		$thumbnailsize = 'permalink';
		if ( array_key_exists( $sidebarname, $side_bar_thumbnail ) ) {
			$thumbnailsize = $side_bar_thumbnail[$sidebarname];
		}
		...
		<div class="productshowcase">
			...
			<?php
			$productcount = $instance['sports-scrollbox'];

			if ( $productcount == NULL ) {
				$productcount = 5;
			}
			$count = 0;
			query_posts( 'cat=' . $instance['category'] . '&showposts=' . $productcount );
			if (have_posts()) :
			while (have_posts()) :
			the_post();
			$count ++;
					...
					<ul>
						...
						<li>
							<div class="sportsscore" style="width:<?php echo $width; ?>px">
								...
								<?php $link = get_post_meta( $post->ID, 'customlink', true );
	...
	function form( $instance ) {
		...
		$defaults = array(
			...
			'sports-scrollbox' => '10',
			'sports-scroll-style' => 'horizontal',
			'sports-speed' => '4000',
		...
		$sidebarname = '';
		if ( $registered_sidebar['name'] ) {
			$instance['sidebarname'] = $registered_sidebar['name'];
			$sidebarname = $instance['sidebarname'];
		}
		if ( array_key_exists( $sidebarname, $side_bar_photo ) ) {
			$instance['photowidth'] = $side_bar_photo[$sidebarname];
		}
		...
		<div class="epg-widget-options-left">
			...
			<p>
				<label
					for="<?php echo $this->get_field_id( 'category' ); ?>"><?php _e( "Select your showcase category:" ); ?></label><br />
			...
			<p>
				<input type="text" id="<?php echo $this->get_field_id( 'sports-scrollbox' ); ?>"
					   name="<?php echo $this->get_field_name( 'sports-scrollbox' ); ?>"
					   value="<?php echo $instance['sports-scrollbox']; ?>" size="2" />Number of products to cycle
				through scrollbox?
			</p>

			<p>
				<select id="<?php echo $this->get_field_id( 'sports-scroll-style' ); ?>"
						name="<?php echo $this->get_field_name( 'sports-scroll-style' ); ?>">
					<option
						value="vertical" <?php if ( 'vertical' == $instance['sports-scroll-style'] ) {
						echo 'selected="selected"';
					} ?>>
						vertical
					</option>
					<option
						value="horizontal" <?php if ( 'horizontal' == $instance['sports-scroll-style'] ) {
						echo 'selected="selected"';
					} ?>>
						horizontal
					</option>
				...
				<select id="<?php echo $this->get_field_id( 'sports-speed' ); ?>"
						name="<?php echo $this->get_field_name( 'sports-speed' ); ?>">
					<option value="2000" <?php if ( '2000' == $instance['sports-speed'] ) {
						echo 'selected="selected"';
					} ?>>2
						Seconds
					</option>
					<option value="3000" <?php if ( '3000' == $instance['sports-speed'] ) {
						echo 'selected="selected"';
					} ?>>3
						Seconds
					</option>
					<option value="4000" <?php if ( '4000' == $instance['sports-speed'] ) {
						echo 'selected="selected"';
					} ?>>4
						Seconds
					</option>
					<option value="5000" <?php if ( '5000' == $instance['sports-speed'] ) {
						echo 'selected="selected"';
					} ?>>5
						Seconds
					</option>
					<option value="6000" <?php if ( '6000' == $instance['sports-speed'] ) {
						echo 'selected="selected"';
					} ?>>6
						Seconds
					</option>
					<option value="7000" <?php if ( '7000' == $instance['sports-speed'] ) {
						echo 'selected="selected"';
					} ?>>7
						Seconds
					</option>
					<option value="8000" <?php if ( '8000' == $instance['sports-speed'] ) {
						echo 'selected="selected"';
					} ?>>8
						Seconds
					</option>
				...
				<select id="<?php echo $this->get_field_id( 'sports-transition' ); ?>"
						name="<?php echo $this->get_field_name( 'sports-transition' ); ?>">
					<option
						value="333" <?php if ( '333' == $instance['sports-transition'] ) {
						echo 'selected="selected"';
					} ?>>
						Fast
					</option>
					<option
						value="666" <?php if ( '666' == $instance['sports-transition'] ) {
						echo 'selected="selected"';
					} ?>>
						Medium
					</option>
					<option
						value="1000" <?php if ( '1000' == $instance['sports-transition'] ) {
						echo 'selected="selected"';
					} ?>>
						Slow
					</option>
		...
		<div class="epg-widget-options">
			...
			<p>
				<input class="colorwellscore<?php echo $number; ?>"
			...
			<script>
				jQuery(document).ready(function () {
					...
					jQuery(".colorwellscore<?php echo $number; ?>").click(function () {
						jQuery('#snocolorpickerscore<?php echo $number; ?>').slideDown()
					});
