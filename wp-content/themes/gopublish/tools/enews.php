...
class eNews_Updates extends WP_Widget {
	...
	function eNews_Updates() {
		$widget_ops = array( 'classname' => 'enews-widget', 'description' => 'Displays eNews &amp; Updates signup form' );
		$control_ops = array( 'width' => 500, 'height' => 250, 'id_base' => 'enews' );
...
<?php	}
	...
	function form($instance) { 
			$defaults = array( 'title' => 'Email Updates', 'text' => 'Enter your email address below to receive our daily email updates', 'widget-style' => get_theme_mod('widget-style'), 'header-color' => get_theme_mod('accentcolor-header'), 'header-text' => '#ffffff', 'widget-border' => '#aaaaaa', 'widget-background' => '#eeeeee', 'border-thickness' => '1px' );
...
<?php $number = $this->number; ?>
		...
		<p>
			<input class="colorwellenews<?php echo $number; ?>" id="<?php echo $this->get_field_id('header-color'); ?>" name="<?php echo $this->get_field_name('header-color'); ?>" type="text" maxlength="7" size="7" value="<?php echo $instance['header-color']; ?>" /> Header Bar <br />
			<input class="colorwellenews<?php echo $number; ?>" id="<?php echo $this->get_field_id('header-text'); ?>" name="<?php echo $this->get_field_name('header-text'); ?>" type="text" maxlength="7" size="7" value="<?php echo $instance['header-text']; ?>" /> Header Bar Text<br />
			<input class="colorwellenews<?php echo $number; ?>" id="<?php echo $this->get_field_id('widget-border'); ?>" name="<?php echo $this->get_field_name('widget-border'); ?>" type="text" maxlength="7" size="7" value="<?php echo $instance['widget-border']; ?>" /> Border<br />
			<input class="colorwellenews<?php echo $number; ?>" id="<?php echo $this->get_field_id('widget-background'); ?>" name="<?php echo $this->get_field_name('widget-background'); ?>" type="text" maxlength="7" size="7" value="<?php echo $instance['widget-background']; ?>" /> Background
		</p>
<script>
		  jQuery(document).ready(function() {
    var f = jQuery.farbtastic('#snocolorpickerenews<?php echo $number; ?>');
    var p = jQuery('#snocolorpickerenews<?php echo $number; ?>').css('opacity', 0.25);
    var selected;
    jQuery('.colorwellenews<?php echo $number; ?>')
  ...
  jQuery(document).ready(function() {
    jQuery('#snocolorpickerenews<?php echo $number; ?>').hide();
    jQuery('#snocolorpickerenews<?php echo $number; ?>').farbtastic(".colorwellenews<?php echo $number; ?>");
    jQuery(".colorwellenews<?php echo $number; ?>").click(function(){jQuery('#snocolorpickerenews<?php echo $number; ?>').slideDown()});
  });
</script>
