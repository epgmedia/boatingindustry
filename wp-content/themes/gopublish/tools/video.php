...
class sno_video extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		if ( ! empty( $totalvideos ) ) {
			...
			<?php $videocount = 0;
			$recent           = query_posts( 'cat=' . $instance['category'] . '&showposts=' . $totalvideos );
			if ( have_posts() ) : while ( have_posts() ) : the_post();
				...
				<?php if ( $videocount <= $instance['number'] ) { ?>
					...
					if ( $video ) {
						?><?php $pattern = "/height=\"[0-9]*\"/";
						$video           = preg_replace( $pattern, "height='$videoheight'", $video );
						$pattern         = "/width=\"[0-9]*\"/";
						$video           = preg_replace( $pattern, "width='100%'", $video );
						echo $video; ?>

						<div
							style="<?php if ( ( ( $instance['show-headline'] == 'on' ) || ( $instance['show-teaser'] == 'on' ) || ( $instance['show-date'] == 'on' ) || ( $instance['show-comments'] == 'on' ) || ( $instance['show-writer'] ) || ( is_user_logged_in() ) ) && ( $instance['remove-padding'] == 'on' ) ) { ?>padding:5px 10px 0 10px;<?php } ?>">

							<?php if ( $instance['show-writer'] == "on" ) {
								$videographer = get_post_meta( $post->ID, videographer, true );
								if ( $videographer ) {
									?><p class="photocredit">Video Credit: <?php echo $videographer; ?></p><?php } ?>
							<?php } ?>
							...
							<?php if ( $instance['show-headline'] == "on" ) {
								...
								<a href="<?php the_permalink() ?>" rel="bookmark"
								   title="Permanent Link to <?php the_title(); ?>"><h3 class="homeheadline"
																					   style="font-size:<?php echo $headlinesize; ?>px; line-height:<?php echo $headlineheight; ?>px;margin-top:0"><?php the_title(); ?></h3>
								</a>
							<?php } ?>
						...
						<?php if ( ( $videocount >= 1 ) && ( $videocount < $totalvideos ) && ( $totalvideos != 1 ) ) { ?><?php if ( $instance['dividing-line'] == 'on' ) { ?>
						<div
							class="storybottom" <?php if ($instance['remove-padding'] == 'on') { ?>style="margin-left: 10px; margin-right: 10px;"<?php } ?>></div><?php } else { ?>
					...
					<div<?php if ( $instance['remove-padding'] == 'on' ) { ?> style="padding-left:10px;padding-right:10px"<?php } ?>>
						...
						<?php if ( $instance['teaser-thumb'] == 'on' ) {
							...
							<?php if ( has_post_thumbnail() ) {
								the_post_thumbnail( 'videothumb', array( 'class' => 'catboxthumb', 'style' => 'margin-bottom:10px; float:' . $thumbplacement . '; margin-' . $thumbplacement . ':0;' ) );
							}
			...
			<?php if ( $instance['view-all'] == 'on' ) { ?>
				...
				<a href="<?php echo cat_id_to_slug( $instance['category'] ); ?>"><p class="viewall"
	...
	function form( $instance ) {
		...
		<?php if ( ( $sidebarname == "Non-Home Sidebar" ) || ( $sidebarname == "Sports Center Sidebar" ) ) {
		...
		<div style="float:left;width:230px;margin-right:20px;border-right:1px solid #aaaaaa;padding-right:10px;">
			...
			<p>
				...
				<input class="checkbox" type="checkbox" <?php if ( $instance['show-headline'] == 'on' )
					echo checked; ?>
			...
			<p>
				<input class="checkbox" type="checkbox" <?php if ( $instance['show-teaser'] == 'on' )
					echo checked; ?>
			...
			<p>
				<input class="checkbox" type="checkbox" <?php if ( $instance['remove-padding'] == 'on' )
					echo checked; ?>
		...
		<div style="float:left;width:210px">
			...
			<p>This widget will display the embed code from the video custom field from stories assigned to the selected
				category.</p>
